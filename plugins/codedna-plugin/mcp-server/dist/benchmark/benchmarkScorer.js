const levels = ["blocked", "cautious", "full", "edge"];
export function scoreBenchmarkRound(input) {
    const threshold = input.threshold ?? 95;
    const failedCases = [];
    const perLevelCounts = Object.fromEntries(levels.map((level) => [level, { total: 0, passed: 0, failed: 0 }]));
    ...
    const perLevelAccuracy = Object.fromEntries(levels.map((level) => [level, percentage(perLevelCounts[level].passed, perLevelCounts[level].total)]));
    return {
...
function toFailedCase(result, reasons) {
    return {
...
function suspectedRootCause(reasons) {
...
function failurePatterns(failedCases) {
