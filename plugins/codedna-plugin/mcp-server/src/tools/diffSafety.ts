...
import type { CodeDnaGuardrails, FinalVerdict, RiskLevel } from "../types/guardrails.js";
...
const apiKeyPatterns: Array<[RegExp, string]> = [
...
export function parseChangeSet(diffText: string, changedFiles: string[] = []): ParsedChangeSet {
...
export function forbiddenTouched(files: string[], forbiddenPatterns: string[]): string[] {
  return files.filter((file) => forbiddenPatterns.some((pattern) => matchesPathPattern(file, pattern)));
}

export function strictAllowedFilesFromText(text: string): string[] {
...
function unrelatedChangedFiles(
  files: string[],
...
function findPatternLabels(text: string, patterns: Array<[RegExp, string]>): string[] {
  const labels: string[] = [];
...
function isLargeUnrequestedRefactor(originalRequest: string, files: string[], text: string): boolean {
...
function requirementMismatch(requirement: RequirementStrand | undefined, text: string, fallbackRequest: string): string[] {
...
function architectureRisk(input: { codex_summary?: string; diff_text?: string }, changes: ParsedChangeSet, largeRefactor: boolean): string[] {
...
function securityRisk(hardcodedSecrets: string[], dangerousCommands: string[], forbiddenFiles: string[], text: string): string[] {
  return uniqueStrings([
...
function performanceRisk(text: string): string[] {
  return uniqueStrings([
