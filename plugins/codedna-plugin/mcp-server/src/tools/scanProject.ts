...
import type { DependencyFile, ProjectProfile } from "../types/projectProfile.js";
...
const dependencyFileKinds: Record<string, string> = {
  "package.json": "node",
...
export async function scanProject(input: ScanProjectInput, memoryStore: MemoryStore): Promise<ScanProjectOutput> {
  ...
  const dependencyFiles = await Promise.all(
    files.filter((file) => dependencyFileKinds[basename(file)]).map((file) => readDependencyFile(root, file))
  );
...
async function readDependencyFile(root: string, file: string): Promise<DependencyFile> {
...
function detectFrameworks(root: string, dependencies: DependencyFile[], files: string[]): string[] {
...
function detectPackageManager(root: string, files: string[], dependencies: DependencyFile[]): string {
...
async function mainDirectories(root: string): Promise<string[]> {
...
function namedDirectories(root: string, files: string[], names: Set<string>): string[] {
...
function protectedFiles(root: string, files: string[]): string[] {
...
async function treeSummary(root: string, maxDepth: number): Promise<string[]> {
  const result: string[] = [];
...
function notes(languageCounts: Map<string, number>, framework: string[], dependencies: DependencyFile[]): string[] {
  const items: string[] = [];
...
function sortedLanguages(languageCounts: Map<string, number>): string[] {
