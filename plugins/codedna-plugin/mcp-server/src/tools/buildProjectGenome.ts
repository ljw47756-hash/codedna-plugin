...
export interface BuildProjectGenomeInput {
  project_path: string;
  project_profile?: ProjectProfile;
  save?: boolean;
}

export interface BuildProjectGenomeOutput {
...
export async function buildProjectGenome(
  input: BuildProjectGenomeInput,
  memoryStore: MemoryStore
): Promise<BuildProjectGenomeOutput> {
...
async function readExistingGenome(projectRoot: string): Promise<Partial<ProjectGenome> | undefined> {
  try {
    const text = await readFile(join(projectRoot, ".codedna", "project-genome.json"), "utf8");
    return JSON.parse(text) as Partial<ProjectGenome>;
  } catch {
    return undefined;
  }
}

function projectId(projectRoot: string, name: string): string {
  return sanitizeFilename(`${name}-${projectRoot.toLowerCase()}`, "project");
}

function projectTypes(profile: ProjectProfile, files: string[]): string[] {
  const types: string[] = [];
...
function isMcpServer(profile: ProjectProfile, files: string[]): boolean {
  return (
    profile.dependency_files.some((file) => file.packages.includes("@modelcontextprotocol/sdk")) ||
    files.some((file) => /(^|[\\/])server\.ts$/i.test(file) && /mcp|server/i.test(file))
  );
}

function architectureStyles(profile: ProjectProfile, files: string[]): string[] {
...
function routingFiles(root: string, files: string[]): string[] {
  return files
...
function apiFiles(root: string, files: string[], profile: ProjectProfile): string[] {
...
function stateManagementFiles(root: string, files: string[]): string[] {
  return files
    .map((file) => toRelative(root, file))
...
function testStrategy(profile: ProjectProfile): string[] {
  const items: string[] = [];
...
function safeEditZones(profile: ProjectProfile): string[] {
  return uniqueStrings([
...
function sensitiveFilesPresent(files: string[]): string[] {
  const names = new Set(files.map((file) => basename(file)));
  return sensitiveFiles.filter((name) => names.has(name));
}

function detectedPatterns(profile: ProjectProfile, files: string[]): string[] {
  return uniqueStrings([
...
function riskAreas(profile: ProjectProfile, files: string[]): string[] {
  return uniqueStrings([
...
function recommendedRules(profile: ProjectProfile): string[] {
  return uniqueStrings([
...
function warnings(profile: ProjectProfile, files: string[]): string[] {
  return uniqueStrings([
