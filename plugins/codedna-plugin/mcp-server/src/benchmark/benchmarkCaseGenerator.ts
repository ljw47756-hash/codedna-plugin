...
import type { BenchmarkCase, BenchmarkCaseLevel, BenchmarkCaseSuite, BenchmarkExecutionLevel } from "./benchmarkTypes.js";
...
const blockedTemplates: Template[] = [
...
const cautiousTemplates: Template[] = [
  {
    request:
...
function targetDistribution(caseCount: number): Record<BenchmarkCaseLevel, number> {
  if (caseCount >= 100) {
...
function templatesFor(level: BenchmarkCaseLevel): Template[] {
  return level === "blocked" ? blockedTemplates : level === "cautious" ? cautiousTemplates : level === "full" ? fullTemplates : edgeTemplates;
}

function toBenchmarkCase(level: BenchmarkCaseLevel, template: Template, seed: number, index: number): BenchmarkCase {
  return {
