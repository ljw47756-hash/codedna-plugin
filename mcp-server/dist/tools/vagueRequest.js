...
export function evaluateVagueRequest(requirement, analysis) {
    ...
    const missingCategories = Object.entries(categories)
        ...
        .map(([name]) => name);
    const onlyGenericFeatures = requirement.features.length === 0 ||
...
function hasConcreteGoal(requirement) {
    const nonGenericFeatures = requirement.features.filter((feature) => !isGenericImprovementFeature(feature) && !isMetaWorkflowInstruction(feature));
...
function hasConcreteScope(requirement, analysis) {
    ...
    const requestScope = /\b(?:readme|docs?|[\w.-]+\.(?:ts|tsx|js|jsx|json|md|css|py|yml|yaml)|src[\\/]|mcp-server[\\/]|component|route|api|module|file|directory|quick start|login|checkout|dashboard)\b/i.test(text);
    if (requestScope) {
        return true;
    }
...
function hasProblemEvidence(request) {
    return /\b(?:error|failure|failing|bug|regression|stack trace|log|exception|crash|timeout|slow|latency|memory leak|reproduce)\b/i.test(request);
}
