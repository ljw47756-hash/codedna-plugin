...
function hasRiskyScope(requirement, analysis, projectProfile) {
    ...
    return /(\.env|package\.json|package-lock\.json|pnpm-lock\.yaml|yarn\.lock|tsconfig\.json|vite\.config|next\.config|pyproject\.toml|api key|token|secret|password|privacy|security|dependency|lockfile|environment variable|不要公开|避免.*抄袭|密钥|令牌|隐私|安全|依赖|锁文件|环境变量|配置文件)/iu.test(text);
}
function complexityScore(requirement, analysis, projectProfile) {
    ...
    let score = 0;
    if (analysis.affected_files.length >= 5)
        score += 2;
    else if (analysis.affected_files.length >= 3)
        score += 1;
    if (analysis.required_modules.length >= 5)
        score += 2;
    else if (analysis.required_modules.length >= 3)
        score += 1;
    if (analysis.implementation_steps.length >= 6)
        score += 2;
    else if (analysis.implementation_steps.length >= 4)
        score += 1;
    if (analysis.risks.length >= 4)
        score += 1;
    if (projectProfile && projectProfile.component_dirs.length + projectProfile.api_dirs.length + projectProfile.test_dirs.length >= 4)
        score += 1;
...
function isTinyTask(requirement, analysis, projectProfile) {
    ...
    return (analysis.affected_files.length <= 1 &&
