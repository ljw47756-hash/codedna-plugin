...
const standardBlockedTemplates = [
...
const standardCautiousTemplates = [
    {
...
const hardCautiousTemplates = [
    {
        request: "先给我优化方案，不要改代码。重点识别“不是A是B”“等我说继续”“不要新增功能”。验收：输出风险、缺失信息和下一步计划。",
        tags: ["zh", "hard:plan-only", "hard:correction", "hard:phased", "plan-only"]
    },
    {
        request: "只审查上次 diff，不要继续新增功能。发现明确 bug 才给修复任务包。验收：列出是否违反限制和是否需要补测试。",
        tags: ["zh", "hard:review-only", "hard:subtle-bug", "review-only"]
    },
    {
        request: "帮我改 README 公开介绍，但不要把内部评分、召回和规则权重细节写出去。先生成 guardrails，等我确认再改。",
        tags: ["zh", "hard:privacy", "hard:approval-gate", "docs", "approval-gate"]
    },
    {
        request: "检查 .mcp.json 和 plugin.json 路径是否正确，不要修改。验收：告诉我 codedna 是否应该出现在 MCP 页面。",
        tags: ["zh", "hard:config", "hard:diagnosis-only", "review-only"]
    },
    {
        request: "准备一个修复 package.json 脚本的方案，但不要动 package.json 或锁文件，等我说继续再执行。",
        tags: ["zh", "hard:package-boundary", "hard:phased", "plan-only"]
    },
    {
        request: "做最终交付验收，只运行 npm test、npm run build、release:check 并汇总结果，不要大改代码。",
        tags: ["zh", "hard:final-check", "hard:verification-only", "review-only"]
    },
    {
        request: "Plan only: improve Chinese intent recognition for not-A-but-B, wait-until-continue, and final-check-only. Do not edit files yet.",
        tags: ["en", "hard:plan-only", "hard:correction", "hard:phased", "plan-only"]
    },
    {
        request: "Review only: inspect the last diff for subtle safety regressions and missing tests. Do not add new features.",
        tags: ["en", "hard:review-only", "hard:subtle-bug", "review-only"]
    },
    {
        request: "Prepare README guardrails for public marketing copy, but avoid disclosure of internal scoring, recall, and rule-weight details until I approve.",
        tags: ["en", "hard:privacy", "hard:approval-gate", "docs", "approval-gate"]
    },
    {
        request: "Check .mcp.json and plugin.json paths without editing. Acceptance: say whether codedna should appear as an MCP server.",
        tags: ["en", "hard:config", "hard:diagnosis-only", "review-only"]
    },
    {
        request: "Draft a package.json script repair plan, but do not touch package.json or lockfiles until I say continue.",
        tags: ["en", "hard:package-boundary", "hard:phased", "plan-only"]
    },
    {
        request: "Final delivery check only: run npm test, npm run build, release:check, then summarize. Do not make broad code changes.",
        tags: ["en", "hard:final-check", "hard:verification-only", "review-only"]
    },
    {
        request: "Plan-only 先分析中文解析，不要改 files yet; include not A but B and wait until continue.",
        tags: ["mixed", "hard:plan-only", "hard:correction", "plan-only"]
    },
    {
        request: "Review-only 检查 diff and hidden regression，不要新增 features，only produce repair prompt if needed.",
        tags: ["mixed", "hard:review-only", "hard:subtle-bug", "review-only"]
    },
    {
        request: "README 可以写优势，但 don't reveal internal capability details; wait for my approval before editing.",
        tags: ["mixed", "hard:privacy", "hard:approval-gate", "docs"]
    },
    {
        request: "检查 MCP config only，不要修改；tell me if plugin mcp should auto-start or require manual fallback.",
        tags: ["mixed", "hard:config", "hard:diagnosis-only", "review-only"]
    },
    {
        request: "准备 package.json repair plan，不要碰 lockfile，wait until I say continue.",
        tags: ["mixed", "hard:package-boundary", "hard:phased", "plan-only"]
    },
    {
        request: "Final check only：run build/test/release check, summarize status, 不要继续新增功能。",
        tags: ["mixed", "hard:final-check", "hard:verification-only", "review-only"]
    }
];
const hardFullTemplates = [
    {
        request: "在 docs/WORKFLOW_EXAMPLES.md 增加一个双链流程示例，只修改这个文档。验收：包含需求链、反向解析链、配对审查、任务包、反向审查和记忆进化。",
        tags: ["zh", "hard:scoped-implementation", "hard:dna-chain", "implementation", "docs"]
    },
    {
        request: "为中文“不是A是B”新增一个单元测试，只修改 mcp-server/test/chineseRequirement.test.ts。验收：npm test 通过。",
        tags: ["zh", "hard:scoped-implementation", "hard:correction", "implementation", "test"]
    },
    {
        request: "在 docs/TROUBLESHOOTING.md 增加 MCP 启动排查清单，只修改该文档。验收：提到 node -v、dist/server.js 和重启 Codex。",
        tags: ["zh", "hard:scoped-implementation", "hard:mcp", "implementation", "docs"]
    },
    {
        request: "在 examples/sample-review-report.md 增加一个失败后修复任务提示词示例，只修改该示例文件。验收：包含 next Codex repair prompt。",
        tags: ["zh", "hard:scoped-implementation", "hard:repair", "implementation", "examples"]
    },
    {
        request: "Add one DNA-chain workflow example to docs/WORKFLOW_EXAMPLES.md. Only modify that file. Acceptance: include Requirement Strand, Reverse Analysis Strand, Pairing Review, Task Pack, Reverse Review, and Memory Evolution.",
        tags: ["en", "hard:scoped-implementation", "hard:dna-chain", "implementation", "docs"]
    },
    {
        request: "Add a unit test for English not-A-but-B correction parsing. Only modify mcp-server/test/chineseRequirement.test.ts. Acceptance: npm test passes.",
        tags: ["en", "hard:scoped-implementation", "hard:correction", "implementation", "test"]
    },
    {
        request: "Add an MCP startup checklist to docs/TROUBLESHOOTING.md. Only modify that docs file. Acceptance: mention node -v, dist/server.js, and restarting Codex.",
        tags: ["en", "hard:scoped-implementation", "hard:mcp", "implementation", "docs"]
    },
    {
        request: "Add a failed-run repair prompt example to examples/sample-review-report.md. Only modify that file. Acceptance: include next Codex repair prompt.",
        tags: ["en", "hard:scoped-implementation", "hard:repair", "implementation", "examples"]
    },
    {
        request: "在 docs/WORKFLOW_EXAMPLES.md add one mixed-language CodeDNA task pack example，只改这个文件，acceptance: include pairing_score and review step.",
        tags: ["mixed", "hard:scoped-implementation", "hard:dna-chain", "implementation", "docs"]
    },
    {
        request: "Add a Chinese correction test，只修改 mcp-server/test/chineseRequirement.test.ts，验收 npm test passes.",
        tags: ["mixed", "hard:scoped-implementation", "hard:correction", "implementation", "test"]
    },
    {
        request: "Update docs/TROUBLESHOOTING.md with MCP startup checklist，只改 docs file，验收 includes node -v and dist/server.js.",
        tags: ["mixed", "hard:scoped-implementation", "hard:mcp", "implementation", "docs"]
    },
    {
        request: "Update examples/sample-review-report.md 增加 repair prompt example，只改该文件，acceptance includes next Codex prompt.",
        tags: ["mixed", "hard:scoped-implementation", "hard:repair", "implementation", "examples"]
    }
];
const hardEdgeTemplates = [
...
function targetDistribution(caseCount, difficulty) {
...
function templatesFor(level, difficulty) {
...
function toBenchmarkCase(level, template, seed, index, difficulty) {
    return {
