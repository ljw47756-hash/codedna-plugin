...
export async function generateBenchmarkReports(input) {
...
${input.runs
        .map((run) => `- ${run.round_id}: accuracy ${run.accuracy}%, failed ${run.failed_cases.length}, patterns ${run.failure_patterns.length ? run.failure_patterns.map((item) => `\`${sanitize(item)}\``).join(", ") : "None"}`)
        .join("\n")}
