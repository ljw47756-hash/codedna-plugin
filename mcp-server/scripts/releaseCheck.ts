...
if (!marketplace.plugins?.some((entry: any) => entry.name === "codedna-plugin" && entry.source?.path === "./")) {
  missing.push("marketplace must include codedna-plugin entry pointing to ./ for repository-root installs");
}
