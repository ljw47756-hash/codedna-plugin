...
import type { FinalVerdict } from "../types/guardrails.js";
import type { OutcomeScore } from "../types/outcomeScore.js";
...
export interface ScoreOutcomeInput {
  ...
  test_plan_result?: Record<string, unknown>;
  codex_output?: string;
}

export type ScoreOutcomeOutput = OutcomeScore;

export async function scoreOutcome(input: ScoreOutcomeInput, _memoryStore: MemoryStore): Promise<ScoreOutcomeOutput> {
...
function requirementScore(input: ScoreOutcomeInput): number {
...
function constraintScore(input: ScoreOutcomeInput): number {
...
function codeQualityScoreFor(input: ScoreOutcomeInput): number {
...
function testScore(input: ScoreOutcomeInput): number {
...
function architectureScore(input: ScoreOutcomeInput): number {
...
function riskScoreFor(input: ScoreOutcomeInput): number {
...
function nextAction(verdict: FinalVerdict, testScoreValue: number, requirementScoreValue: number, constraintScoreValue: number): string {
  if (verdict === "blocked") {
...
function reviewIssues(reviewReport?: Record<string, unknown>): number {
  const checks = (reviewReport as { checks?: Array<Record<string, unknown>> } | undefined)?.checks ?? [];
