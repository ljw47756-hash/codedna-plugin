...
function semanticGroups(pairType: string): Array<{ requirement: string[]; analysis: string[]; confidence: number }> {
  ...
  if (pairType === "Constraint <-> Risk") {
    return shared.filter(
      (group) =>
        group.requirement.includes("constraint") ||
...
function evaluateSafetyGate(requirement: RequirementStrand): { blocked: boolean; cautious: boolean; warnings: string[] } {
  ...
  const missingScopeBlock =
    /(not specified|did not say|not provided|forgot to provide|没有给|没给|没说|未提供).{0,50}(scope|batch|content|files?|commands?|acceptance|范围|内容|文件|命令|验收)/iu.test(request) ||
    /(final check only|最终检查|最终验收).{0,80}(which commands or files|acceptance commands|检查哪些文件|验收命令|没说|没有给)/iu.test(request);
  const realMissingScopeBlock =
    /(scope|batch|content|files?|commands?|acceptance|范围|内容|文件|命令|验收|第一批|第二批).{0,50}(not specified|没有给|没给|没说|未提供)/iu.test(request) ||
  ...
  const unsafeCompletionBlock =
    /mark it complete without asking|without asking any questions|直接说完成|直接标记完成|不用问我|不要问我/iu.test(request) &&
    /(rewrite|optimi[sz]e|fix all|repair every|whole|entire|architecture|优化整个|修复所有|所有失败|重写|架构)/iu.test(request);
  const realPlanOnlyOrApproval =
    /\b(without editing|without changing files|do not change files|only tell me|config only)\b/i.test(request) ||
...
function hasPackageFileBoundary(request: string): boolean {
  return (
    /\b(?:avoid|do not|don't|without|must not|no)\b.{0,50}\b(?:package files?|package-manager files?|package manager files?|dependency files?|lockfiles?|lock files?)\b/iu.test(request) ||
