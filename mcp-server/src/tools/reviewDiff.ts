...
export interface ReviewDiffOutput extends DiffReview {
  artifact_path?: string;
}

export async function reviewDiff(input: ReviewDiffInput, memoryStore: MemoryStore): Promise<ReviewDiffOutput> {
...
function reviewIdFrom(request: string): string {
  const stamp = new Date().toISOString().replace(/[-:]/g, "").replace(/\.\d{3}Z$/, "Z");
  return `codedna-diff-review-${stamp}-${sanitizeFilename(request, "diff")}`.slice(0, 150);
}

function repairPrompt(input: ReviewDiffInput, requiredFixes: string[]): string {
