import type { ActivatedEffect, CodexAssistanceStep, DnaAlignment, RuleWeightAdjustment } from "../types/pairingResult.js";
import { tokens, uniqueStrings } from "../tools/common.js";
import type { CaseLibrary, RetainedEffect } from "./caseLibrary.js";

export const basePairWeights: Record<string, number> = {
  "Goal <-> Task": 20,
  "Constraint <-> Risk": 18,
...
const familyToPairType: Record<string, string> = {
...
export function scoreAdjustmentFromEffects(activatedEffects: ActivatedEffect[], missingCount: number, unmatchedCount: number): number {
...
export function dnaAlignment(score: number): DnaAlignment {
  return {
...
export function codexAssistanceSteps(score: number): CodexAssistanceStep[] {
...
export function scoreExplanation(
  baseScore: number,
  finalScore: number,
  activatedEffects: ActivatedEffect[],
  caseCounts: { success: number; failure: number; public: number },
  adjustment: number
): string[] {
  return [
...
function scoreEffect(effect: RetainedEffect, query: string, queryTokens: string[], families: string[]): number {
  let score = 0;
  const effectText = [
...
function matchedTerms(effect: RetainedEffect, queryTokens: string[], families: string[]): string[] {
...
function pairTypeForFamily(family: string): string {
  return familyToPairType[family] ?? "Goal <-> Task";
}

function fitWeight(fit: string): number {
  if (fit === "strong") {
    return 1.8;
  }
  if (fit === "medium") {
