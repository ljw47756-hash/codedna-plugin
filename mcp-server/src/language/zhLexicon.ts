...
export type TaskMode = "plan-only" | "review-only" | "implementation";
export type ClauseLabel = "feature" | "constraint" | "preference" | "acceptance" | "privacy" | "phased" | "correction";
...
zhFeatureHints.push(
  "添加", "新增", "创建", "生成", "实现", "支持", "修复", "优化", "重构", "更新",
  "检查", "审查", "扫描", "保存", "上传", "安装", "启用", "解析", "迁移", "替换",
  "换成", "进入", "完善", "方案", "指导", "排查", "注册", "打包", "发布", "部署",
  "文档", "说明", "任务包", "报告", "测试计划", "回归"
);
zhConstraintHints.push(
  "不要", "别", "不再", "不许", "不能", "不用", "不得", "禁止", "避免", "只允许",
  "只做", "只改", "只能", "必须", "务必", "一定要", "保留", "不要改", "不要修改",
  "不要提交", "不要上传", "不要继续", "不要缩减", "先不要", "先不用", "除非",
  "等待我说", "等我说", "不要公开", "不要暴露", "不要泄露", "避免抄袭", "防止抄袭",
  "范围内", "不能加", "不要加", "不大改"
);
zhPreferenceHints.push(
  "偏好", "风格", "简单", "简洁", "清晰", "一致", "紧凑", "可读", "用中文",
  "用英文", "中文描述", "英文", "一步步", "先给方案", "不要太详细", "避免暴露",
  "保密", "说明", "推荐", "建议", "适合", "优先", "最好", "真实", "稳定"
);
zhAcceptanceHints.push(
  "验收", "验证", "校验", "检查", "审查", "执行后", "测试", "回归测试", "运行",
  "通过", "完成后", "最后", "输出", "告诉我", "逐项打勾", "结论", "结果",
  "编译", "构建", "跑一遍", "确认", "列出", "总结", "报告", "对照", "验收成果",
  "通过率"
);
zhPlanOnlyHints.push(
  "先给方案", "只给方案", "先不用改", "先不要改", "不用改代码", "不要改代码",
  "不要改功能代码", "不改代码", "先不用提交", "只分析", "只排查", "只做诊断",
  "先排查", "先检查", "先看原因", "先定位原因", "先不用再改"
);
zhReviewOnlyHints.push(
  "最终检查", "最终验收", "交付检查", "审查结果", "检查是否", "检查隐私数据",
  "风险结论", "给风险结论", "不要继续新增", "不要大改", "只做验收",
  "只做最终检查", "验收阶段"
);
zhImplementationHints.push(
  "开始优化", "开始开发", "直接实现", "帮我修复", "修复", "生成代码", "写完整代码",
  "提交并推送", "上传到 GitHub", "开始改", "修改", "只改", "构建", "打包",
  "运行 npm run build"
);
zhPrivacyHints.push(
  "不要公开", "不要暴露", "避免别人抄袭", "避免抄袭", "保密", "不要泄露",
  "不要把能力具体说出去", "不要把所有内部能力", "内部能力具体说出去", "不要说出去",
  "照着抄"
);
...
export function classifyClause(clause: string): ClauseClassification {
  const labels = new Set<ClauseLabel>();
...
export function detectTaskMode(request: string): TaskMode {
  if (
    containsAny(request, zhPlanOnlyHints) ||
  ...
  if (
    containsAny(request, zhReviewOnlyHints) ||
    isValidationOnlyRequest(request) ||
    isInspectionOnlyRequest(request) ||
    /\b(review only|only review|diff only|final check|delivery check|do not add new features|do not continue development)\b/iu.test(request) ||
    /\bonly\s+review\b/iu.test(request)
  ) {
    return "review-only";
  }
...
function isValidationOnlyRequest(request: string): boolean {
  const isEnglishValidation =
    /\b(run|rerun|check|validate)\b/iu.test(request) &&
  ...
  return (
    /(重新运行|再次运行|重跑|跑一遍|跑一次)/u.test(request) &&
    /(告诉我|给我|输出|汇总|总结).{0,12}(结果|结论|日志|通过情况)/u.test(request) &&
    !/(修复|修改|实现|生成|提交|推送|改代码|写入|更新)/u.test(request)
  );
}

function isInspectionOnlyRequest(request: string): boolean {
  const isEnglishInspection =
    (
    (
      /\b(check|inspect|audit|review)\b.{0,60}\b(privacy|secret|token|api\s*key|risk|memory)\b/iu.test(request) &&
  ...
  return (
    /(重点检查|检查|确认|核对)/u.test(request) &&
    /(告诉我|给我|输出|汇总|总结|是否需要|是否)/u.test(request) &&
    !/(修复|修改|实现|生成|提交|推送|改代码|写入|更新|创建|新增)/u.test(request)
  );
}

export function extractCorrections(text: string): CorrectionDirective[] {
  const corrections: CorrectionDirective[] = [];
  const patterns = [
    /不是\s*([^，。；,.]+?)\s*[，,]?\s*(?:而是|是)\s*([^，。；,.]+)/gu,
...
export function isPhasedInstruction(text: string): boolean {
  return (
    /先.+?(再|然后|最后)/u.test(text) ||
...
export function isPrivacyInstruction(text: string): boolean {
  return containsAny(text, zhPrivacyHints);
}

export function taskModeNote(mode: TaskMode): string {
  if (mode === "plan-only") {
...
function addIf(labels: Set<ClauseLabel>, rules: string[], label: ClauseLabel, rule: string, condition: boolean): void {
  if (condition) {
    labels.add(label);
    rules.push(rule);
  }
